...
def test_edit_product_persists_menu_fields(tmp_path):
    ...
    try:
        resp = client.post(
            "/admin/products/nasi_goreng",
            data={
                "spice_level": "3",
                "ing_en": "rice, chicken, extra sambal",
                "ing_ko": "밥, 닭고기, 삼발소스 추가",
                "ing_ms": "",
        ...
        nasi_goreng = {p.id: p for p in store.list_products("demo")}["nasi_goreng"]
        assert nasi_goreng.spice_level == 3
        assert nasi_goreng.ingredients["en"] == "rice, chicken, extra sambal"
        assert nasi_goreng.ingredients["ko"].startswith("밥, 닭고기")
        assert "ms" not in nasi_goreng.ingredients  # empty field dropped
        assert nasi_goreng.allergens == ("egg", "peanuts")  # normalized lowercase
        assert nasi_goreng.available is True
    finally:
        _close(store)
...
def test_edit_product_unchecked_available_sets_false(tmp_path):
    ...
    try:
        client.post(
            "/admin/products/hainan_chicken_rice",
            data={"spice_level": "0", "allergens": ""},  # no 'available' -> unchecked
        )
        hainan_chicken_rice = {p.id: p for p in store.list_products("demo")}["hainan_chicken_rice"]
        assert hainan_chicken_rice.available is False
    finally:
        _close(store)
...
def test_qa_page_shows_pending_entry(tmp_path):
    client, store = _client(tmp_path)
    try:
        store.add_pending_qa("demo", "Do you have rendang?", {"en": "Yes, in season."})
        html = client.get("/admin/qa").text
        assert "Do you have rendang?" in html
...
def test_approve_pending_qa_makes_it_grounding(tmp_path):
    client, store = _client(tmp_path)
    try:
        entry = store.add_pending_qa("demo", "Do you have rendang?", {"en": "old"})
        resp = client.post(
            f"/admin/qa/{entry.id}/approve",
            data={"question": "Do you sell rendang?", "answer_en": "Yes, when in season!", "answer_ko": "", "answer_ms": ""},
        )
        assert resp.status_code == 200
        ...
        assert approved[entry.id].question == "Do you sell rendang?"
